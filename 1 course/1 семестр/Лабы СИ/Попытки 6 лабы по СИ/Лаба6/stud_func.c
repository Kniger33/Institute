...
void write_student_in_base(Student *base, int size_of_base, char *way) {
    ...
    for (i = 0; i < size_of_base; i++) {
        ...
        fprintf(q, "%s ", base[i].second_name);
...
void add_student(Student **base, int *size_of_base) {
    ...
    *base = (Student*)realloc(*base, (*size_of_base + 1) * sizeof(Student));
    ...
    if(select_RB(*base, &((*base + *size_of_base)->rb), *size_of_base) != NULL) {
        printf("Студент с таким номером зачётки уже есть!\n");
        return;
    }
    ...
    strcmp((*base + *size_of_base)->second_name, cur_second_name);

    (*base + *size_of_base)->first_name = (char*)malloc((strlen(cur_first_name) + 1) * sizeof(char));
    strcmp((*base + *size_of_base)->first_name, cur_first_name);

    (*base + *size_of_base)->patronym = (char*)malloc((strlen(cur_patronym) + 1) * sizeof(char));
    strcmp((*base + *size_of_base)->patronym, cur_patronym);

    printf("\nВведите оценку по математике: ");
    scanf("%d", &((*base + *size_of_base)->math));

    printf("\nВведите оценку по русскому: ");
    scanf("%d", &((*base + *size_of_base)->rus_lang));

    printf("\nВведите оценку по английскому: ");
    scanf("%d", &((*base + *size_of_base)->eng_lang));

    printf("\nВведите оценку по истории: ");
    scanf("%d", &((*base + *size_of_base)->history));

    (*base + *size_of_base)->average_mark = ((*base + *size_of_base)->math + (*base + *size_of_base)->rus_lang + (*base + *size_of_base)->eng_lang + (*base + *size_of_base)->history) / 4;

    printf("%lf",(*base + *size_of_base)->average_mark );
