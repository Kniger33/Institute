...
yyoutput          : String;      (* output string for put_char *)
yyline            : String;      (* current input line *)
yylineno, yycolno : Integer;     (* current input position *)
yytext            : String[255];      (* matched text (should be considered r/o) *)
...
procedure yyMemoInit(Src,Dst,Err,Dbg: Tmemo);
    {apg added for Memos accessing }
implementation

procedure fatal ( msg : String );
  (* writes a fatal error message and halts program *)
  begin
    yyErrMemo.lines.add(msg);
    halt(1);
  end(*fatal*);
...
function get_char : Char;
  var i : Integer;
  begin
    if (bufptr=0) and (yylineno < yySrcMemo.Lines.Count) then
      begin
        yyline := yySrcMemo.lines[yylineno];
