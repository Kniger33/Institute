...
int NodesInTree(BinaryTreeNode *&binaryTreeNode)
{
    ...
    subtreeNodes = leftChildren + rightChildren;
    ...
    return subtreeNodes == 0 ? 1 : subtreeNodes + 1;
}
...
void TreeHeight(BinaryTreeNode *&binaryTreeNode)
{
    ...
    while (currentNode != nullptr)
    {
        ...
        {
            valueNodes.push(currentNode->_right->_value);
                ...
                )
        {
            if(!valueNodes.empty())
            {
                currentNode = SearchInTree(binaryTreeNode, valueNodes.top());
                valueNodes.pop();
                if(curHeight > height)
                {
                    height = curHeight;
