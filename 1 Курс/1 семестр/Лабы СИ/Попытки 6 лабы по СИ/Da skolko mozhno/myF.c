...
int add_student() {
    ...
    while(num != student1.number) {
        num = student1.number;
        fscanf(fp, "%d%s%s%s%s%s%s%s%d%d%d%d%f", &student1.number, &student1.sec_name, &student1.name, &student1.patronym, &student1.discp[0], &student1.discp[1], &student1.discp[2], &student1.discp[3], &student1.mark[0], &student1.mark[1], &student1.mark[2], &student1.mark[3], &student1.Smark);
...
int del_student() {
    ...
    for (i = 0; i < count; i++) {
        scanf(fp, "%d%s%s%s%s%s%s%s%d%d%d%d%f", &student.number, &student.sec_name, &student.name, &student.patronym, &student.discp[0], &student.discp[1], &studentent[i].discp[2], &studentent[i].discp[3], &student.mark[0], &student.mark[1], &student.mark[2], &student.mark[3], &student.Smark);
    ...
    for (i = 0; i < count; i++)
    {
        ...
        fprintf(fp, "%d %s %s %s %s %s %s %s %d %d %d %d %.2f ", studentent[i].number, studentent[i].sec_name, studentent[i].name, studentent[i].patronym, studentent[i].discp[0], studentent[i].discp[1],  studentent[i].discp[2], studentent[i].discp[3], studentent[i].mark[0], studentent[i].mark[1], studentent[i].mark[2], studentent[i].mark[3], studentent[i].Smark);
        }
        else {
            result = 0;
        }
...
void select_fio_of_student() {
    ...
    while(point != student.number) {
        ...
        else if (strcmp(s, student.name) == 0) {
            result = 0;
            printf("\nДжедай юный найден:\n");
            printf("%d %s %s %s %s %s %s %s %d %d %d %d %.2f \n", student.number, student.sec_name, student.name, student.patronym, student.discp[0], student.discp[1],  student.discp[2], student.discp[3], student.mark[0], student.mark[1], student.mark[2], student.mark[3], student.Smark);
        }
...
void mark_of_student(int count ) {
   ...
   // printf("Введите оценку джедая юного: ");
    ...
    switch (select) {
        ...
        case 1: for(i = 0; i < count; i++) {
                    if (student.Smark <= cur_mark && student.Smark >= 0 && student.Smark <= 5) {
                        flag = 0;
